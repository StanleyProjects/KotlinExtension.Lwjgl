...
internal class TrueTypeRenders(
    ...
    private fun getAtlasOld(ftFace: FT_Face, fontHeight: Int): Atlas {
...
//        val padding = 0
    ...
    private fun getAtlas(
        ...
        val atlasHeight = getAtlasHeight(fontHeight = fontHeight.toDouble())
...
//                val atlas = getAtlasOld(ftFace = ftFace, fontHeight = fontHeight)
                ...
                val scale = fontHeight.toDouble() / (atlas.ascender - atlas.descender)
                println("$Tag: atlas:scale($fontHeight): $scale")
    ...
    private fun onRenderAtlas(
        ...
        val dY = atlas.ascender - atlas.descender
        canvas.vectors.draw(
            color = Color.Gray.copy(alpha = 0.5f),
            vector = pointOf(x = 0.0, y = y + dY * scale).let { it + it.copy(x = ps.width) },
            lineWidth = 1.0,
        )
    }
...
//        )
        canvas.texts.draw(
            ...
            pointTopLeft = pointOf(x = x, y = y * 4),
            text = text,
        )
    }
